    ...
    private void openMediaPlayer() {
        ...
        try {
            ...
            if(mUrl.contains("http")){
                final File file = new File(getDiskCacheDir(mContext) + "/" + mUrl.substring(mUrl.lastIndexOf("/")+1));
                if(file.exists() && isRePlay){
                    mMediaPlayer.setDataSource(getDiskCacheDir(mContext) + "/" + mUrl.substring(mUrl.lastIndexOf("/")+1));
                }else{
